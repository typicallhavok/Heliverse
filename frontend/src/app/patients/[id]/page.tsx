...
const calculateTotalProtein = (dietChart: DietCharts) => {
  const breakfastProtein = parseNutrients(
    dietChart.breakfast.nutrients
  ).protein;
...
export default function PatientPage() {
  ...
  const handleDelete = async () => {
    if (window.confirm("Are you sure you want to remove this patient?")) {
      try {
        await axios.delete(`${process.env.NEXT_PUBLIC_BACKEND_URL}/patients/${params.id}`, {
          withCredentials: true,
        });
    ...
    <div className="space-y-4 max-w-6xl mx-auto px-4 py-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-card p-4 rounded-lg shadow-lg border border-border/50">
          <h2 className="text-lg font-semibold mb-3 text-foreground text-center">
            Basic Info
          </h2>
      ...
      {patient.DietCharts?.[0] && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          {["breakfast", "lunch", "dinner"].map((meal) => {
            const currentMeal = patient.DietCharts[0][meal as MealType];

            return (
              <div
                key={meal}
                className="bg-card p-4 rounded-lg shadow-lg border border-border/50"
              >
                <h3 className="text-lg font-semibold mb-3 text-foreground capitalize text-center">
                  {meal}
                </h3>
                <div className="space-y-3">
                  <div className="bg-secondary/20 p-3 rounded-lg">
                  ...
                  <div className="bg-secondary/20 p-3 rounded-lg">
                    ...
                    <p className="font-medium">{currentMeal.calories} kcal</p>
                  </div>
                  {currentMeal.restrictions && (
